    ...
    <>
      ...
      <section className="relative h-screen flex items-center justify-center">
        ...
        <div className="relative z-10 text-center max-w-4xl mx-auto px-4">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8">
            <Link
              href="/estimate"
              className="bg-blossom hover:bg-blossom-dark text-white px-8 py-4 rounded-lg font-semibold text-lg transition-colors"
            >
              Price Mulch
            </Link>
            <a
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={item.step} className="relative bg-white shadow-md border border-soil-dark/50 rounded-xl p-8 pt-12 text-center">
                <div className="absolute -top-5 left-1/2 -translate-x-1/2 flex h-10 w-10 items-center justify-center rounded-full bg-blossom text-white font-bold text-lg shadow-md">
                  {item.step}
                </div>
                <h3 className="text-xl font-outfit font-bold text-sand mb-2">{item.title}</h3>
                <p className="text-bark leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-bark text-center mb-10 max-w-2xl mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mb-12">
            {serviceAreas.map((area) => (
              <Link
                ...
                className="bg-white hover:bg-blossom hover:text-white text-sand font-medium px-4 py-2 rounded-full transition-colors text-sm"
              >
                {area.name}
              </Link>
            ))}
      ...
      <section className="bg-blossom py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/estimate"
              className="bg-white text-blossom hover:bg-blossom-light px-8 py-4 rounded-lg font-semibold text-lg transition-colors"
            >
              Price Mulch
            </Link>
            <a
